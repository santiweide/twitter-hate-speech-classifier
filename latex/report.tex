...
\begin{itemize}
    \item \textbf{NER+BERT:} A pipeline that first extracts Named Entities (NER) and then uses BERT for classification. \footnote{This is called Amphate in our code since I reference their paper for the implementation.}
...
\begin{itemize}
    ...
    \item \textbf{Efficiency:} Here resource computation efficiency is focused, and the metrics are latency is analyzed by torch profile. Only inference time efficiency is analyzed, and training efficiency is not measured here in the report. Throughput and GPU memory consumption are also important and due to time limit and GPU resource limitation they are not performed in this experiment. See ~\ref{tab:latency_summary} for result.
...
\begin{table}[htbp]
\centering
\caption{Comparative Latency of Main Inference Functions}
\label{tab:latency_summary}
\begin{tabular}{llcc}
\toprule
\textbf{Model Profiler} & \textbf{Main Function Name} & \textbf{Latency (CPU Total)} & \textbf{GPU Time (CUDA Total)} \\
\midrule
NER Profiler & \texttt{ner\_pipeline\_inference} & 11.968s & 3.068s \\
BERT Profiler & \texttt{model\_forward\_pass} & 9.217s & 8.589s \\
RoBERTa Profiler & \texttt{model\_forward\_pass} & 10.009s & 10.796s \\
\bottomrule
\end{tabular}
\end{table}
